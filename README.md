...
<p align="center">
  <img src="docs/images/apercu-portables.jpg" alt="The same library on a phone:
  the filter and sort rows fold behind a single button, leaving the platform
  selector, the search field and the grid."
  width="300">
</p>

<!-- Both pictures are generated by `outils/captures.py` from an invented
     library — no real title, no third-party artwork, nobody's collection.
     Regenerate them there rather than by hand: a screenshot taken from a real
     installation says what its owner has, and ships somebody else's cover art
     with it. That happened twice. -->
