...
def main():
    ...
    try:
        ...
        for url, attendu in (
            ...
            t("deduit %s" % attendu, notify.guess(url) == attendu,
              "%s -> %s" % (url, notify.guess(url)))
        ...
        for service, correcte in formes.items():
            ...
            cfg = cfg_avec({"id": "1", "url": BASE + "/x", "service": service,
                            "evenements": ["tache_ok"]})
            n = notify.send("tache_ok", "Titre", "Corps", "ok", cfg, wait=True)
            r = recu()
        ...
        cfg = cfg_avec({"id": "1", "url": BASE + "/sujet", "service": "ntfy",
                        "evenements": ["tache_ok"]})
        notify.send("tache_ok", "Pokémon terminé", "Corps", "error", cfg, wait=True)
        r = recu()
        ...
        t("aucun evenement coche = tous",
          notify.send("tache_echec", "T", "", "error", cfg, wait=True) == 1)

        # --- failures do not propagate --------------------------------------
        reussi, raison = notify.probe(BASE + "/refuse")
        t("un 500 est rapporte comme un echec", reussi is False, raison)
        reussi, raison = notify.probe("http://127.0.0.1:1/rien")
        t("un service injoignable ne leve pas", reussi is False, raison)
        reussi, raison = notify.probe("file:///etc/passwd")
        ...
        t("une destination en panne ne fait pas echouer l'envoi",
          notify.send("tache_ok", "T", "", "ok", cfg, wait=True) == 1)
        ...
        t("le nombre de destinations est borne",
          len(notify.destinations(beaucoup)) == notify.MAX_DESTINATIONS)
    finally:
        srv.terminate()
    print("   ------------------------------------------------")
