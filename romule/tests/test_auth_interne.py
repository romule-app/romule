...
c, b, _ = appel("/api/compte-creer", {"email": "alex@example.org",
                                      "mdp": "grand cheval bleu 42", "nom": "Alex"})
...
c, b, _ = appel("/api/compte-creer", {"email": "alex@example.org", "mdp": "autre chose longue"})
...
print("   -- 5. connexion --")
c, b, _ = appel("/auth/connexion", {"email": "alex@example.org", "mdp": "mauvais"}, forme=True)
t("mauvais mot de passe refuse", c == 401)
t("message unique", b"Email ou mot de passe incorrect" in b)
c, b, _ = appel("/auth/connexion", {"email": "alex@example.org",
                                    "mdp": "grand cheval bleu 42"}, forme=True)
t("connexion acceptee", c == 200, c)
c, b, _ = appel("/api/job")
t("acces retabli", c == 200, c)
c, b, _ = appel("/auth/moi")
t("session lisible", (js(b).get("session") or {}).get("email") == "alex@example.org", js(b))
