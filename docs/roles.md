...
Thirty-seven routes are reserved server-side. They fall into seven groups:
...
- **choose where the service reads and writes on the host** — the folder
  picker, the library location, the list of consoles, and the
  [vault](sauvegardes.md). Browsing the host's filesystem is a disclosure
  primitive, so it is treated as one; declaring a console changes eight
  settings at once, which is the same thing by another route; and a backup
  reads the whole library and writes gigabytes to a path the caller names —
  both halves are administrator business;
...
thirty-seven routes against an ordinary account — for internal accounts and for
SSO sessions alike.
